...
If `.assistant-config.md` already exists:
...
The skill adds to `.assistant-config.md`:
...
1. **Read** `.assistant-config.md` at start (if exists; follow migration preflight in `references/config-migration.md`)
